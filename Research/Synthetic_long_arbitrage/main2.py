    ...
    def run(self, startDate, endDate, baseSym, indexSym):
        ...
        for timeData in df.index: 
            ...
            if self.humanTime.date() >= expiryDatetime.date() :
                MonthlyExpiry = getExpiryData(self.timeData+86400, baseSym)['MonthlyExpiry']
                expiryDatetime = datetime.strptime(MonthlyExpiry, "%d%b%y").replace(hour=15, minute=20)
            ...
            if (timeData in df.index):
                    ...
                    strikePrice = None
                    putSym = self.getPutSym(self.timeData, baseSym, df.at[lastIndexTimeData[1], "c"], MonthlyExpiry)
                    callSym = self.getCallSym(self.timeData, baseSym, df.at[lastIndexTimeData[1], "c"], MonthlyExpiry)
                    data_put = None
                    data_call = None
...
if __name__ == "__main__":
    ...
    endDate = datetime(2025, 4, 30, 15, 00)
    ...
    df["Synthetic"] = (df["Call_Premium"] - df["Put_Premium"]) + df['Strike_Price']

    # Generate full minute range between startDate and endDate
    full_range = pd.date_range(start=startDate, end=endDate, freq='T')
    df = df.reindex(full_range)

    # Fill missing rows with "Data not Found"
    for col in ["Strike_Price", "putSym", "Put_Premium", "callSym", "Call_Premium", "Synthetic"]:
        df[col] = df[col].where(df[col].notna(), "Data not Found")

    df.index.name = "Datetime"
    df.to_csv(output_file)

    print(f"CSV file '{output_file}' created successfully with Datetime as index and missing minutes marked.")
